#' Visualize question #27: indicator design.
#' References:
...
#'
#' `visualize_q27_indicator_design()` visualizes question data.
#'
#' @return plot output.
#'
#' @examples
#' visualize_q27_indicator_design();
#'
#' \dontrun{
#' visualize_q27_indicator_design();
#' }
#'
#' @export
visualize_q27_indicator_design <- function() {
  ...
  series_titles = c(
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R1), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R2), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R3), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R4), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R5), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R6), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R7), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R8), "IndDesign - "),
    stringr.tools::str_unprefix(sjlabelled::get_label(question_data$R9), "IndDesign - ")
  );
  ...
  q29_legend = data.frame(series_codes, series_titles);
  colnames(q29_legend) = c("Item", "Statement");
  ...
  question_data$R6 = ifelse(question_data$R1 == 6, NA, question_data$R6);
  question_data$R7 = ifelse(question_data$R2 == 6, NA, question_data$R7);
  question_data$R8 = ifelse(question_data$R3 == 6, NA, question_data$R8);
  question_data$R9 = ifelse(question_data$R4 == 6, NA, question_data$R9);
